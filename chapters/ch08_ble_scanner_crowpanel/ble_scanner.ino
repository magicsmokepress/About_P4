...
// ─── Hardware init (same boilerplate) ─────────────────────────────
static void lvgl_flush_cb(lv_display_t *disp,
                          const lv_area_t *a, uint8_t *px) {
    esp_lcd_panel_handle_t p = g_lcd->getHandle();
    if (p) {
        esp_err_t r;
        do {
            r = esp_lcd_panel_draw_bitmap(p, a->x1, a->y1,
                                          a->x2 + 1, a->y2 + 1, px);
...
static void init_hardware() {
    ...
    g_touch = new TouchGT911(tb, LCD_WIDTH, LCD_HEIGHT,
    ...
    uint8_t *buf1 = (uint8_t *)heap_caps_malloc(bsz,
                                                  MALLOC_CAP_SPIRAM);
    assert(buf1);

    lvgl_disp = lv_display_create(LCD_WIDTH, LCD_HEIGHT);
    lv_display_set_flush_cb(lvgl_disp, lvgl_flush_cb);
    lv_display_set_buffers(lvgl_disp, buf1, NULL, bsz,
...
class ScanCallbacks : public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        strncpy(devices[device_count].name, name.c_str(),
                sizeof(devices[0].name) - 1);
        strncpy(devices[device_count].addr, addr,
                sizeof(devices[0].addr) - 1);
...
static void build_ui() {
    ...
    lv_obj_set_style_bg_color(btn, lv_color_hex(0x4466CC),
                              LV_STATE_PRESSED);
    lv_obj_set_style_radius(btn, 8, 0);
    lv_obj_add_event_cb(btn, [](lv_event_t *e) { do_scan(); },
    ...
    lv_obj_set_style_text_font(list_lbl,
        &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(list_lbl,
        lv_color_hex(0xAABBCC), 0);
...
static void update_ui() {
    if (ble_ready) {
        lv_label_set_text(status_lbl,
            ble_scanning ? "Scanning..."
                         : "BLE Ready (via C6 co-processor)");
        lv_obj_set_style_text_color(status_lbl,
            ble_scanning ? lv_color_hex(0x4488FF)
                         : lv_color_hex(0x00FF88), 0);
    } else {
        lv_label_set_text(status_lbl,
            "BLE FAILED - not supported on this core version");
        lv_obj_set_style_text_color(status_lbl,
    ...
    if (device_count > 0) {
        ...
        for (int i = 0; i < device_count
                        && pos < (int)sizeof(list_buf) - 100; i++) {
            pos += snprintf(list_buf + pos, sizeof(list_buf) - pos,
                "%2d. %-25s %s  %ddBm\n",
                i + 1, devices[i].name,
                devices[i].addr, devices[i].rssi);
        }
        list_buf[pos] = 0;
...
void setup() {
    Serial.begin(115200);
    delay(500);
    Serial.println("CrowPanel P4 - BLE Scanner");
    ...
    if (ble_ready) {
        Serial.println("[BLE] Working! Auto-scanning...");
        do_scan();
    }
