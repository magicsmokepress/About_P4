...
// ─── ArduTFLite int8 workaround ─────────────────────────
// ArduTFLite's modelSetInput/modelGetOutput assume float tensors.
// Person detection uses int8. We bypass the API and write directly.
extern TfLiteTensor *tflInputTensor;
extern TfLiteTensor *tflOutputTensor;

// ─── Camera config (from Ch26) ──────────────────────────
...
// Detection state
static float rolling_confidence[ROLLING_AVG_FRAMES] = {0};
static int rolling_idx = 0;
static float smoothed_confidence = 0.0f;
static bool person_detected = false;
static uint32_t inference_ms = 0;

// Auto-exposure (from Ch26)
static uint8_t current_exposure = 0xC0;

// ─── SCCB register access via Wire (from Ch26) ─────────
...
// ─── SC202CS register table (from Ch26, unchanged) ─────
...
// ─── Software Bayer demosaic (from Ch26, unchanged) ─────
...
// ─── Auto-exposure (from Ch26, unchanged) ───────────────
static void auto_expose(const uint8_t *raw, int w, int h) {
    int cx = w / 2, cy = h / 2;
    uint32_t sum = 0;
    int count = 0;
    ...
    if (avg < 45) current_exposure = min(0xF0, current_exposure + 4);
    if (avg > 75) current_exposure = max(0x20, current_exposure - 4);
    cam_write_reg(0x3e01, current_exposure);
}

// ─── Preprocess: center-crop 96x96 grayscale int8 ───────
static void preprocess_frame(const uint16_t *rgb) {
    int x_off = (DISP_H - MODEL_INPUT_W) / 2;   // 272
    int y_off = (DISP_V - MODEL_INPUT_H) / 2;   // 132
    int8_t *input_data = tflInputTensor->data.int8;

    for (int y = 0; y < MODEL_INPUT_H; y++) {
        for (int x = 0; x < MODEL_INPUT_W; x++) {
            uint16_t px = rgb[(y_off + y) * DISP_H + (x_off + x)];
            uint8_t r = (px >> 11) << 3;
            uint8_t g = ((px >> 5) & 0x3F) << 2;
            uint8_t b = (px & 0x1F) << 3;
            uint8_t gray = (r * 77 + g * 150 + b * 29) >> 8;
            input_data[y * MODEL_INPUT_W + x] = (int8_t)(gray - 128);
        }
    }
}
...
static void draw_overlay() {
    // Draw center crop rectangle (where the model is looking)
    int x_off = (DISP_H - MODEL_INPUT_W) / 2;
    int y_off = (DISP_V - MODEL_INPUT_H) / 2;
    uint16_t box_color = person_detected ? TFT_GREEN : TFT_DARKGREY;
    M5.Display.drawRect(x_off - 1, y_off - 1,
                        MODEL_INPUT_W + 2, MODEL_INPUT_H + 2, box_color);

    // Status bar background
    M5.Display.fillRect(0, 0, DISP_H, 28, TFT_BLACK);

    // Detection result
    M5.Display.setTextColor(person_detected ? TFT_GREEN : TFT_WHITE);
    M5.Display.setTextSize(2);
    M5.Display.setCursor(8, 6);
    if (person_detected) {
        M5.Display.printf("PERSON  %.0f%%", smoothed_confidence * 100);
    } else {
        M5.Display.printf("No person  %.0f%%", smoothed_confidence * 100);
    }

    // Inference time (right-aligned)
    M5.Display.setTextColor(TFT_YELLOW);
    M5.Display.setCursor(DISP_H - 140, 6);
    M5.Display.printf("%lu ms", inference_ms);
}

// ─── CSI callbacks (from Ch26) ──────────────────────────
...
// ─── Setup ──────────────────────────────────────────────
void setup() {
    // 1. M5.begin() - display, touch, IO expanders, Wire
    auto cfg = M5.config();
    M5.begin(cfg);
    ...
    Serial.println("\n=== Tab5 Person Detection ===");
    Serial.println("SC202CS + TFLite Micro on ESP32-P4\n");
    ...
    if (!raw_buf || !rgb_buf) {
        Serial.println("[ERR] Failed to allocate camera buffers!");
        while (1) delay(1000);
    }

    // 6. Allocate tensor arena in PSRAM
    tensor_arena = (uint8_t *)heap_caps_malloc(TENSOR_ARENA_SIZE,
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!tensor_arena) {
        Serial.println("[ERR] Failed to allocate tensor arena!");
        while (1) delay(1000);
    }
    Serial.printf("[ML] Tensor arena: %d KB in PSRAM\n",
        TENSOR_ARENA_SIZE / 1024);

    // 7. Initialize TFLite model
    if (!modelInit(person_detect_model, tensor_arena, TENSOR_ARENA_SIZE)) {
        Serial.println("[ERR] modelInit() failed!");
        while (1) delay(1000);
    }
    Serial.printf("[ML] Model loaded: %d bytes\n", person_detect_model_len);
    Serial.printf("[ML] Input: [%d, %d, %d, %d] int8\n",
        tflInputTensor->dims->data[0], tflInputTensor->dims->data[1],
        tflInputTensor->dims->data[2], tflInputTensor->dims->data[3]);
    Serial.printf("[ML] Output: [%d, %d] int8\n",
        tflOutputTensor->dims->data[0], tflOutputTensor->dims->data[1]);

    // 8. ISP - RAW8 passthrough
    ...
    isp_cfg.output_data_color_type = ISP_COLOR_RAW8;
    isp_cfg.h_res = CAM_H;
    isp_cfg.v_res = CAM_V;
    ESP_ERROR_CHECK(esp_isp_new_processor(&isp_cfg, &isp));
    ESP_ERROR_CHECK(esp_isp_enable(isp));

    // 9. CSI - 1 lane, 400 Mbps, RAW8 passthrough
    ...
    Serial.println("[ML] Person detection active\n");
}
...
void loop() {
    ...
    if (esp_cam_ctlr_receive(cam_handle, &t, 100) == ESP_OK) {
        // Auto-expose every 60 frames (from Ch26)
        if (++frame_count % 60 == 0) {
        ...
        // Preprocess: extract center 96x96 grayscale int8
        preprocess_frame(rgb_buf);

        // Run inference
        uint32_t t_start = millis();
        if (modelRunInference()) {
            inference_ms = millis() - t_start;

            // Read int8 output directly (bypassing broken modelGetOutput)
            int8_t *out = tflOutputTensor->data.int8;
            float no_person_score = (out[0] + 128) / 255.0f;
            float person_score    = (out[1] + 128) / 255.0f;

            // Rolling average over N frames
            rolling_confidence[rolling_idx] = person_score;
            rolling_idx = (rolling_idx + 1) % ROLLING_AVG_FRAMES;
            float sum = 0;
            for (int i = 0; i < ROLLING_AVG_FRAMES; i++) {
                sum += rolling_confidence[i];
            }
            smoothed_confidence = sum / ROLLING_AVG_FRAMES;
            person_detected = (smoothed_confidence >= DETECTION_THRESHOLD);
        } else {
            inference_ms = 0;
            Serial.println("[ML] Inference failed!");
        }

        // Display viewfinder + overlay
    ...
    // FPS and memory stats every 5 seconds
    if (millis() - last_fps >= 5000) {
        float fps = frames * 1000.0 / (millis() - last_fps);
        Serial.printf("%.1f FPS | Inference: %lu ms | Person: %.0f%% | "
            "PSRAM: %u KB free\n",
            fps, inference_ms, smoothed_confidence * 100,
