/**
 * CrowPanel Advanced 7" ESP32-P4 — I2C Bus Scanner
 *
 * Scans the I2C1 header (GPIO 45 SDA, GPIO 46 SCL) for all
 * connected devices.  NO display initialization — just serial
 * output — so Wire.h works without driver_ng conflicts.
 *
 * Expected devices on this bus:
 *   0x5D or 0x14 — GT911 touch controller (on-board)
 *   0x76 or 0x77 — BME280 / BMP280 (if connected)
 *   0x68 or 0x69 — MPU6050 (if connected)
 *   0x38        — AHT10 (if connected)
 *   0x23        — BH1750 (if connected)
 *
 * BOARD SETTINGS:
...
void setup() {
    ...
    if (found == 0) {
        ...
        Serial.println("  - Some sensors need time to boot — press RESET to re-scan");
    } else {
