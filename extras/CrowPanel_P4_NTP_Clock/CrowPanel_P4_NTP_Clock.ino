 ...
 *
 * Connects to WiFi, syncs time via NTP, and displays a live clock
 ...
 *   Partition: Huge APP (3MB No OTA/1MB SPIFFS) — same as WiFi scanner
 *
 * USAGE:
 *   1. Set your WiFi credentials below (WIFI_SSID / WIFI_PASS)
...
#define WIFI_SSID "MAGA"
#define WIFI_PASS "l@t!tud3"
...
// ─── Setup — EXACT same order as working WiFi scanner ─────────────
...
void setup() {
  ...
  // Display + touch first (same as WiFi scanner)
  init_hardware();
  build_ui();
  ...
  // WiFi init — EXACT same pattern as working WiFi scanner
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (status_lbl) {
      lv_label_set_text(status_lbl, LV_SYMBOL_CLOSE "  WiFi failed");
      lv_obj_set_style_text_color(status_lbl, lv_color_hex(0xFF5252), 0);
    }
    if (date_lbl) lv_label_set_text(date_lbl, "No WiFi — clock will show --:--");
  }
